...
# Note: Code here is planned to be reworked using a simplier formumla and Tailwind CSS
...
DEFAULT_TDS_THRESHOLDS = {  # Default thresholds colors for TDS
    "min": {"value": 0, "color": "bg-blue-200"},  
    "low": {"value": 500, "color": "bg-green-200"}, 
    "mid": {"value": 1000, "color": "bg-red-200"}, 
    "max": {"value": 2000, "color": "bg-red-500"},  
}

DEFAULT_TURBIDITY_THRESHOLDS = {  # Default thresholds/colors for turbidity
    "min": {"value": 0, "color": "bg-blue-200"},  
    "low": {"value": 5, "color": "bg-green-200"}, 
    "mid": {"value": 50, "color": "bg-red-200"},  
    "max": {"value": 100, "color": "bg-red-500"},  
}
...
# Setters and getters for thresholds
...
# Color intensity increases or decreases based on the percentile within the threshold range
...
# :return: A Tailwind CSS class.
    try:
        ...
        ):  # Find the range current_temp falls into
            lower_threshold = sorted_thresholds[i]
            upper_threshold = sorted_thresholds[i + 1]
            if (
                lower_threshold[1]["value"]
                <= current_temp
                <= upper_threshold[1]["value"]
            ):
                return lower_threshold[1]["color"]

        if (
            current_temp < sorted_thresholds[0][1]["value"]
        ):  # If temp is below the lowest threshold
