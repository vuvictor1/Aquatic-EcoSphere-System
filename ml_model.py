...
# File: ml_model.py
# Description: Machine learning model for predicting sensor data.
...
# Prepare dataset with a larger window of past values
def prepare_data(sensor_type: str) -> tuple:
    """
    Args: sensor_type (str): Type of sensor data to prepare.
    Returns: tuple: X_train, X_test, y_train, y_test, latest_data
    """
    data = get_all_data()
...
# Train model using Random Forest Regressor model
...
def get_predictions(sensor_types):
    ...
    Args: sensor_types (list): List of sensor types.

    Returns: dict: Dictionary with sensor types as keys and tuples of predictions, accuracy, and last reading as values.
    """
    predictions = {}
    for sensor_type in sensor_types:
        ...
        if X_train is not None:
            ...
            predictions[sensor_type] = (next_prediction, accuracy, last_reading)
    return predictions
...
for sensor_type in sensor_types:
    ...
    # Predictions first (Keep off to save performance)
