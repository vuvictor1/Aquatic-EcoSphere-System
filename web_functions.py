...
def eco_header():  # Header for the web interface
    with ui.header().style('background-color: #3AAFA9; padding: 10px 20px; position: static;'):
        ...
        with ui.row().style('gap: 10px;'):  # Buttons style
            ...
            # settings dropdown 2 options
            with ui.dropdown_button(icon='settings', auto_close=True):
                ui.item('Thresholds', on_click=lambda: ui.navigate.to(
                    '/settings'))  # settings redirect
                ui.item('Species', on_click=lambda: ui.navigate.to(
                    '/species'))  # species redirect
