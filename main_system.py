...
def home_page(): # Home page function
    ...
    with ui.right_drawer().style('background-color: #6C757D; align-items: center;'): # Right drawer
        ...
        for notice in notices: # Iterate through each notice
            ui.chat_message(notice, name='Robot', avatar='https://robohash.org/ui').style(LABEL_STYLE) # chat message
        ...
        ui.chat_message(disclaimer, name='Robot', avatar='https://robohash.org/ui').style(LABEL_STYLE) # chat message
...
storage_secret = os.getenv('STORAGE_SECRET') # get storage secret from .env file
