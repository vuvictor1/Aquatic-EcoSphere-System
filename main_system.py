...
def home_page(): # Home page function
    ...
    with ui.right_drawer().style('background-color: #6C757D; align-items: center;'): # Right drawer 
        ...
        notices = [ 
            '1. Timers update periodically in intervals of 10mins. (Set to 10secs for debugging and testing only)',
            '2. Recommendations are suggestions, not mandatory.',
            '3. Specify species before proceeding, otherwise default values will be used.',
    ...
    lottie_url = 'https://lottie.host/33548596-614d-4e89-a0a8-69126f02a92a/EmTPHrDT7l.json' # lottie file url
    ...
    with ui.row().style('justify-content: center; width: 100%; margin-top: 20px;'):
        for card_type in ['Alerts', 'Reminders', 'Recommendations']:
            with ui.column().classes('card').style('align-items: center;'):
                ...
                ui.label('Content: WIP...').style(LABEL_STYLE)
    ...
    with ui.dialog() as date_dialog:
        ...
        date_picker.on('update:model-value', update_date_input) # Update date input when date picker value changes

    # Button to open the date range selection dialog
    with ui.row().style('justify-content: center; width: 100%; margin-top: 20px;'):
        ui.button('Select Date Range', on_click=lambda: date_dialog.open()).style('background-color: #3AAFA9; color: #FFFFFF; margin-top: 10px;')
    ...
    with ui.row().style('justify-content: center; width: 100%; margin-top: 10px;'):
        ui.button('Filter Data', on_click=lambda: (
            generate_graphs(graph_container, get_all_data(*date_input.value.split(' - ')) if date_input.value else get_all_data()),
            date_dialog.close()
    ...
    with ui.row().style('justify-content: center; width: 100%;'): # Refresh graphs button
        ui.button('Refresh Graphs', on_click=lambda: generate_graphs(graph_container)).style('background-color: #3AAFA9; color: #FFFFFF;')
