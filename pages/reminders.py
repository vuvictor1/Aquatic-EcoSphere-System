...
def get_upcoming_task(rows):  # Task with the least amount of days for dashboard
    filtered_rows = [row for row in rows if int(
        row['frequency']) != 99]  # ignore the example task
...
def reminders_page():  # Renders the reminders page
    ...
    columns = [  # Columns for the table to sort the task
        ...
        {'name': 'frequency',
            'label': 'Frequency (Days)', 'field': 'frequency', 'sortable': True},
    ]
    rows = load_data()  # Load data from file

    with ui.column().classes('flex justify-center items-center w-full h-screen p-4 md:h-3/4 md:mt-20'):  # Center the column
        with ui.element('div').classes('outline_label p-6 md:p-12 bg-gray-800 rounded-lg shadow-lg w-full max-w-md'):  # Responsive container
            ...
            with ui.table(title='Maintenance Tasks', columns=columns, rows=rows, selection='multiple', pagination=10).classes('w-full mb-4') as table:
                    ...
                    with table.row():
                        ...
                        with table.cell():  # Button for new tasks
                            ui.button(on_click=lambda: (
                                add_new_task(
                                    table, rows, new_task.value, new_frequency.value)
                            ), icon='add').props('flat fab-mini')

                        with table.cell():  # Input for new task
                            new_task = ui.input('Task').classes('bg-gray-200 px-4 py-2 w-full mb-4')

                        with table.cell():  # Input for new frequency
                            new_frequency = ui.input('Frequency').classes('bg-gray-200 px-4 py-2 w-full mb-4')
            ui.button('Remove', on_click=lambda: (
                table.remove_rows(table.selected),
                update_task(rows),
                save_data(rows)  # save data to file
            )).classes('w-full bg-red-500 text-white py-2 rounded mb-4')  # take out items
