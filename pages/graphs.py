    ...
    if data:  # Generate graphs if data is available
        ...
        for sensor_type in desired_order:
            if sensor_type in data:  # Check if sensor type is in the data
                ...
                timestamps = [
                    entry["timestamp"].strftime("%m-%d %H:%M") for entry in values
                ]  # extract timestamps
                sensor_values = [
                ...
                with graph_container:  # Create a graph container
                    ui.echart(
                        {  # Create an ECharts graph
                            "title": {
                                "text": sensor_type.title(),
                                "textStyle": {"color": "#FFFFFF"},
                            },
                            "tooltip": {
                                "trigger": "axis",
                                "textStyle": {"color": "#rgb(16, 15, 109)"},
                            },
                            "xAxis": {
                                "type": "category",
                                "data": timestamps,
...
def graphs_page(graph_container):  # Graphs page for the web interface
    ...
    with ui.dialog() as date_dialog:  # Create a dialog for selecting date range
        with ui.column().classes("bg-gray-800 p-4 sm:p-8 rounded-lg my-2"):
            ...
            def update_date_input():  # Update the date input with the selected range
                ...
                date_input.value = (
                    f"{selected_range['from']} - {selected_range['to']}"
                    if selected_range
                    and "from" in selected_range
                    and "to" in selected_range
                    else None
                )
            ...
            with ui.row().classes("mt-0 my-2"):  # Create a row for the filter button
                ui.button(
                    ...
                    on_click=lambda: (
                        generate_graphs(
                            graph_container,
                            get_all_data(*date_input.value.split(" - "))
                            if date_input.value
                            else get_all_data(),
                        ),
    ...
    graph_container = ui.row().classes(
        "graph-container justify-center items-center bg-gray-800 p-4 my-2 mx-auto w-full max-w-screen-md"
    )
